...
void mouse(int button, int state, int x, int y) {
    if (button == GLUT_LEFT_BUTTON && state == GLUT_DOWN) {
        lightPos.x = (x - windowWidth / 2) / (windowWidth / 20);
        lightPos.y = (windowHeight / 2 - y) / (windowHeight / 20);
...
int main(int argc, char **argv) {
    ...
    mainWindow = glutCreateWindow("Illumination and shadow");
